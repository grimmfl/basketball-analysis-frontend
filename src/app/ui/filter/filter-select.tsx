import {ComparatorOperator, Model, OrderDir, SearchRequest} from "@/app/fixed-models";
...
export default function FilterSelect<T extends Model>({model, onChange, disabled, defaultId}: {
    model: FilterSelectModel,
    ...
    const defaultValue = defaultId != null
        ? data.find(d => d.id === defaultId) ?? null
        : null;
    ...
    const request: SearchRequest = {
        ...
        filter: [[
            {attribute: "search", comparator: {operator: ComparatorOperator.Contains, value: search}}
        ]],
        take: -1
    };
    ...
    function search(value: string) {
        ...
        request.filter = [[
            {attribute: "search", comparator: {operator: ComparatorOperator.Contains, value: value}}
        ]];
    ...
    return (
        <div>
            <input data-tooltip-id="tooltip"
                   ...
                   placeholder="Search ..."
                   value={selected == null ? searchValue.length > 0 ? searchValue : defaultValue != null ? config.selector(defaultValue) : "" : config.selector(selected)}
                   onFocus={() => {
                       setSearchValue("");
                       search("");
                       setSelected(null)
            ...
            ></input>
            <Tooltip openEvents={{focus: true, mouseover: false}}
                     closeEvents={{blur: true, mouseout: false}}
                     id="tooltip" place="bottom" clickable
                     style={{backgroundColor: "var(--background)", color: "var(--foreground)", padding: 0}}
                     border="1px solid gray"
                     noArrow={true}
                     ref={tooltip}
            >
                <div className="flex-col h-72 overflow-y-auto bg-black">
                    {isLoading ? <Spinner></Spinner> : data.map((t, idx) =>
                        <div>
                            <button
                                key={idx}
                                onClick={() => {
