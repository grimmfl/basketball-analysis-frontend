...
import {ComparatorOperator, OrderDir, SearchComparator, SearchRequest} from "@/app/fixed-models";
import React, {useEffect, useRef, useState} from "react";
import clsx from "clsx";
import {Observable} from "@/app/observable";
import {Translations} from "@/app/translations";
import {getProperty, removeDuplicates, resolveProperty, zip} from "@/app/utils";
...
export default function Table<T>({config}: { config: TableConfig<T> }) {
    ...
    function updateSearchRequest(request: SearchRequest, prop: string) {
        if (request?.order_by === prop) {
            return {
                ...request,
                order_dir: request.order_dir === OrderDir.Asc ? OrderDir.Desc : OrderDir.Asc
            };
        } else {
            return {
                ...request,
    ...
    function changeSearchValue(value: string) {
        ...
        newFilter.push({name: "search", key: "search", comparator: {operator: ComparatorOperator.Contains, value: value}});
    ...
    function search(request: SearchRequest, custom_external_filters?: Filter[], custom_filters?: Filter[]) {
    ...
    if (config.searchRequest$ != null)
        config.searchRequest$.subscribe(r => {
            const external = r.filter.map(f => ({key: f[0].attribute, name: "", comparator: f[0].comparator}));
    ...
    const [searchRequest, setSearchRequest] = useState(config.initialSearchRequest);
    ...
    useEffect(() => {
        setExternalFilters(config.initialSearchRequest.filter.map(f => ({
            key: f[0].attribute,
            name: "",
        ...
        <div>
            {config.showSearch
                ? <input placeholder="Search ..."
                         ...
                         onChange={(evt) => changeSearchValue(evt.target.value)}></input>
                : <div></div>
            }
            <div className="flex justify-between">
                        ...
                        <table className="min-w-full" id="table" ref={tableRef}>
                            ...
                            <tr className="text-left border-b-4 border-b-gray-900">
                                {config.columns?.map((c, idx) =>
                                    <th key={idx} className={c.headerClassName ?? clsx("p-3 w-auto", {
                                        "hover:cursor-pointer": c.sortable
                                    })}
                                        onClick={() => c.sortable ? sort(c.column) : {}}>
                                        <div className={`flex ${getHeaderAlignment(c)}`}>
                                            <span className="mr-2">{c.name}</span>
                                            <SortIcon prop={c.column} request={searchRequest}></SortIcon>
                                        </div>
                                    </th>)}
                            </tr>
                            </thead>
                            <tbody className="pr-3">
                            {isLoading ? <tr><td><Spinner></Spinner></td></tr> : data.map((entry: T, idx) =>
                                <tr key={idx} className={clsx("border-b border-b-gray-900", {
                                    "hover:cursor-pointer hover:bg-gray-900": config.onRowClick != null
                                })}
                                    onClick={() => config.onRowClick != null ? config.onRowClick(entry) : {}}>
                                    {config.columns.map((c, cidx) =>
                                        <td key={cidx}
                                            className={`p-3 ${getAlignment(c)}`}>{getColumnValue(entry, c)}</td>
                                    )}
                                </tr>
                            )}
                            </tbody>
                ...
                <div hidden={!config.showFilter} className="w-1/2 max-h-screen h-screen">
                    <button className="border border-gray-700 p-3 min-w-full mb-2 hover:bg-gray-900"
                            onClick={() => search(searchRequest)}>Apply Filters
                    </button>
