...
async function getTeamName(id: string): Promise<string> {
    const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/teams/${id}`);
...
async function getPlayerName(id: string): Promise<string> {
    const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/players/${id}`);
...
const PATHS: Path[] = [
    {
        path: "teams", resolve: async (_: string) => "teams", children: [
            {
                path: "{team_id}", resolve: async (id: string) => (await getTeamName(id)), children: [
                    {
                        path: "players", resolve: async (_: string) => "players", children: [
                            {
                                path: "{player_id}",
    ...
    return <div className="flex">
        {names.map((c, idx) =>
            <span key={idx} className="font-bold flex">
                {idx < names.length - 1 ? <a className="underline hover:cursor-pointer"
                                             onClick={() => routeTo(idx)}>{prettifyName(c)}</a> : prettifyName(c)}

                {idx < names.length - 1 ? <ChevronRightIcon className="w-4 mx-1"></ChevronRightIcon> : ''}
            </span>
        )}
    </div>;
}