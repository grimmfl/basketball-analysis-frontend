...
export function zip<T1, T2>(items1: T1[], items2: T2[]): { item1: T1, item2: T2 }[] {
    if (items1.length === 0 || items2.length === 0) return [];

    return [{item1: items1[0], item2: items2[0]}].concat(zip(items1.slice(1), items2.slice(1)));
}

export function removeDuplicates<T>(items: T[], keySelector: (item: T) => any): T[] {
    return items.filter((i, idx) => items.findIndex(i_ => keySelector(i) === keySelector(i_)) === idx);
}